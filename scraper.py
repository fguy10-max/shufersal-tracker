...
def parse_promo_price(desc, regular_price):
    ...
    # Pattern 2: "ב2- ב 65"
    if not unit_price:
        m = re.search(r'ב\s*\d+\s*[-–]\s*ב\s*(\d+(?:[.,]\d+)?)', desc)
        if m:
...
def extract_promos(roots):
    ...
    for root in roots:
        for promo in root.iter('Promotion'):
            desc      = (promo.findtext('PromotionDescription') or promo.findtext('RewardDescription') or '').strip()
            desc      = " ".join(desc.split()) # Normalize whitespace (removes newlines/extra spaces)
...
def apply_promos_to_products(products, promos_dict):
    """
    Robust central handler for merging promotions into products.
    Ensures that "fake" discounts (promoPrice >= price) do not pollute the frontend.
    """
    cnt = 0
    for p in products:
        if p['barcode'] in promos_dict:
            p.update(promos_dict[p['barcode']])
            cnt += 1
            
        # Cleanup invalid XML promoPrice where the "deal" is equal to/higher than shelf price
        promo_price_xml = p.get('promoPrice')
        if promo_price_xml and promo_price_xml >= p['price']:
            p['promoPrice'] = None
            promo_price_xml = None
            
        eff = parse_promo_price(p.get('promo') or '', p['price'])
        eff_price = round(eff, 2) if eff else promo_price_xml
        
        # Only set effectivePrice to something lower than regular price
        if eff_price and eff_price < p['price']:
            p['effectivePrice'] = eff_price
        else:
            p['effectivePrice'] = p['price']
            p['promoPrice'] = None  # Clear useless data to save UI logic and JSON bytes
...
def scrape_shufersal(store_id, service, folder_id):
    ...
    cnt = apply_promos_to_products(products, pd)
    print(f'  {cnt} מבצעים מיושמים')
    return products, {}
...
def get_trend(bc, sh):
    ...
    # CRUCIAL BUG FIX: If no meaningful change found, do NOT return a fake previous price!
    # Returning `prev` here if it equals `curr` causes UI to cross out identical prices.
    if prev is None:
